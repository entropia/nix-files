...
{
  config =
    lib.mkIf (lib.pathIsRegularFile (inputs.self + "/secrets/hosts/${config.networking.hostName}.yaml"))
      {
        ...
        services.borgbackup.jobs.zweitwohnsitz = {
          ...
          encryption = {
            mode = "repokey-blake2";
            passCommand = "cat ${
              config.sops.secrets."hosts/${config.networking.hostName}/backup_zweitwohnsitz_password".path
            }";
          };
          environment = {
